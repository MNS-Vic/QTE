...
from qte_core.events import EventType, SignalEvent
...
class SimpleTickStrategy:
    ...
    def __init__(self, event_loop, symbols, data_provider, price_threshold=0.5, position_size=100):
        """
        初始化策略
        ...
        参数:
            ...
            data_provider (DataProvider): 数据提供者
            price_threshold (float): 价格变动阈值
            position_size (int): 每次交易的数量
        """
        ...
        # 记录每个交易品种的连续上涨/下跌次数
        self.up_count = {symbol: 0 for symbol in symbols}
        self.down_count = {symbol: 0 for symbol in symbols}
        # 记录持仓状态: 1=多头, -1=空头, 0=无持仓
        self.positions = {symbol: 0 for symbol in symbols}
        
        # 注册市场事件处理程序
        self.event_loop.register_handler(EventType.MARKET, self.on_market_event)
    ...
    def on_market_event(self, event):
        ...
        参数:
            event (MarketEvent): 市场事件
        """
        if not hasattr(event, 'data_type') or event.data_type != 'TICK':
            return
        ...
        # 更新连续上涨/下跌计数
        if price_change > 0:
            self.up_count[symbol] += 1
            self.down_count[symbol] = 0
        elif price_change < 0:
            self.down_count[symbol] += 1
            self.up_count[symbol] = 0
        
        # 执行交易逻辑
        if self.positions[symbol] == 0:  # 当前无持仓
            if self.up_count[symbol] >= 3 and price_change > self.price_threshold:
                # 连续上涨3次且价格变动大于阈值，做空
                self.event_loop.add_event(SignalEvent(
                    symbol=symbol, 
                    signal_type="SHORT", 
                    target_quantity=self.position_size
                ))
                self.positions[symbol] = -1
            elif self.down_count[symbol] >= 3 and abs(price_change) > self.price_threshold:
                # 连续下跌3次且价格变动大于阈值，做多
                self.event_loop.add_event(SignalEvent(
                    symbol=symbol, 
                    signal_type="LONG", 
                ...
                self.positions[symbol] = 1
        elif self.positions[symbol] == 1:  # 当前持有多头
        ...
        elif self.positions[symbol] == -1:  # 当前持有空头
            if price_change > self.price_threshold:
                # 价格上涨超过阈值，平仓
                self.event_loop.add_event(SignalEvent(
                    symbol=symbol, 
