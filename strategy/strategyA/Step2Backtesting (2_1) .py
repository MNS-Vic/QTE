...
#
dict_weight=dict([(k,None) for k in ['s1_3','s2_1','s2_2','s2_3','s3_1','s4_1','s4_2']])
weight0=create_decayratio(mode='Sector')
for kw in dict_weight.keys():
    ratio=weight0[k]
    ftratio=ratio*100
    tempweight=ret.applymap(lambda x:np.nan)
    tempweight.loc[tempweight.index[0],:]=ftratio
    tempweight=tempweight.fillna(method='ffill')
    dict_weight[kw]=tempweight   

#%%Bench2
for kw in list(dict_weight.keys()):
    weight=dict_weight[kw]
    dfx['weight']=weight.stack()
    for i in range(benchday,dfx.index.levels[0].size,gapday):
        gapused=gapday if i+gapday<dfx.index.levels[0].size else dfx.index.levels[0].size-i
        date=dfx.index.levels[0][i]#start date of OOS
        print(date,gapused)
        #
        train=dfx.loc[idx[dfx.index.levels[0][i-benchday:i],:],:]
        test=dfx.loc[idx[dfx.index.levels[0][i:i+gapused],:],:]
        ...
        for k,v in fs_rd.items():
            ...
            savepath=r'/home/heshuangji/FactorResearch/ModelSave/SectorRatio/%s'%(kw)
            if not os.path.exists(savepath):os.mkdir(savepath)
            pkl_save(savepath+'/lgb%s_%s.pkl'%(k,date), result)


#%%analysis2
#-----------num_iteration analysis---------
result=dict( [(kw,dict([(k,[]) for k in numtree])) for kw in list(dict_weight.keys())[1:] ])
for kw in list(dict_weight.keys())[1:]: 
    weight=dict_weight[kw]
    dfx['weight']=weight.stack()
    savepath=r'/home/heshuangji/FactorResearch/ModelSave/SectorRatio/%s'%(kw)
    #
    ...
    for k in numtree:
        for i in range(benchday,dfx.index.levels[0].size,gapday):
            ...
            #           
            try:
                data=pkl_load(savepath+'/lgb%s_%s.pkl'%(0.5,date))
            except:
                continue
            temptrain=[];temptest=[]
            for _,gbm in enumerate(data['model']):
                ...
                #
                temp=gbm.predict(X_train[col],num_iteration=k)  
    ...
    for k in numtree:
        ...
        res_test=pd.concat(predict_test[k]).unstack()-rev_num
        #
        ...
        calc_pnl(xval,yval,wval,title='weight(%s)_numtree%s_train'%(kw,k),standard=False)  
        #
        ...
        usecol=(yval*xval).cumsum().min().sort_values().index[:]#年化Ret/MDD
        calc_pnl(xval.loc[xval.index[:],usecol],yval.loc[xval.index[:],usecol],
                 wval.loc[xval.index[:],usecol],title='weight(%s)_numtree%s_test_col(%s)'%(kw,k,round(len(usecol)/yval.shape[1],2)),standard=False)    
k=100
tempyres=[[],[],[]]
for kw in list(dict_weight.keys())[1:]: 
    weight=dict_weight[kw]
    dfx['weight']=weight.stack()
    #    
    xval=pd.concat(result[kw][k]).unstack()-rev_num
    wval=dfx.loc[idx[dfyres[kw].index,:],'weight'].unstack().fillna(method='bfill') 
    tempyres[0].append(wval*xval)
    tempyres[1].append(wval[wval>20]*xval)
    tempyres[2].append(wval[wval>50]*xval)
for  i in range(len(tempyres)):
    temp=pd.concat(tempyres[i],axis=1)
    xval=temp.groupby(temp.columns,axis=1).mean()
    yval=dfx.loc[idx[temp.index,:],'yret'].unstack()
    usecol=(yval*xval).cumsum().min().sort_values().index[:5]#年化Ret/MDD
    calc_pnl(xval.loc[xval.index[:],usecol],yval.loc[xval.index[:],usecol],
             title='weight(plan%s)_numtree%s_test_col(%s)'%(i,k,round(len(usecol)/yval.shape[1],2)),standard=False)    
