...
for i in range(benchday,dfx.index.levels[0].size,gapday):
    ...
    for k,v in fs_rd.items():
        ...
        pkl_save(r'/home/heshuangji/FactorResearch/ModelSave/RollingDay2(s2_2)/lgb%s_%s.pkl'%(k,date), result)
...
for k in numtree:
    for i in range(benchday,dfx.index.levels[0].size,gapday):
        ...
        #
        try:
            data=pkl_load(r'/home/heshuangji/FactorResearch/ModelSave/RollingDay2(s2_2)/lgb%s_%s.pkl'%(0.5,date))
        except:
            continue
        temptrain=[];temptest=[]
...
for k in numtree:
    ...
    usecol=(yval*xval).cumsum().min().sort_values().index[-30:]#年化Ret/MDD
...
for k in overlap:
    for i in range(benchday+k*gapday,dfx.index.levels[0].size-gapday,gapday):
        ...
        #
        try:
            data=pkl_load(r'/home/heshuangji/FactorResearch/ModelSave/RollingDay2(s2_2)/lgb%s_%s.pkl'%(0.5,dateoverlap))
        except:
            continue
        temptrain=[];temptest=[]
        for _,gbm in enumerate(data['model']):
