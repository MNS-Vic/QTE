...
class QTEVirtualExchange:
    ...
    def __init__(self):
        ...
        self.accounts = {
            'USDT': Decimal('100000.0'),  # 初始资金10万USDT
            'BTC': Decimal('0.0')
        }
    ...
    async def _execute_trade(self, order: QTEOrderData, fill_price: Decimal):
        ...
        print(f"💰 账户余额: USDT={self.accounts['USDT']:.2f}, BTC={self.accounts.get('BTC', 0):.4f}")
...
async def main():
    ...
    print("   -> vnpy Gateway -> 交易策略 -> 订单撮合 -> 账户更新")
    print("=" * 60)
    ...
    try:
        ...
        end_date = "2024-01-01 09:30:00"  # 30分钟的数据
