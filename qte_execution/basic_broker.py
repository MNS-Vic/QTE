...
from qte_core.events import OrderEvent, FillEvent, EventType, MarketEvent, OrderDirection, OrderType
from qte_core.event_loop import EventLoop
from qte_execution.interfaces import BrokerSimulator, CommissionModel, SlippageModel
...
class BasicBroker(BrokerSimulator):
    ...
    def submit_order(self, order: OrderEvent) -> None:
        ...
        目前主要处理市价单。
        """
        app_logger.info(f"经纪商收到订单: {order.symbol} {order.direction} {order.quantity} @ {order.order_type}")
...
if __name__ == '__main__':
    from qte_core.event_loop import EventLoop
    from qte_core.events import OrderDirection, OrderType, EventType
    from datetime import datetime, timedelta
